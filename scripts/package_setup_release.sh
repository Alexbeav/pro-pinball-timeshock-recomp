...
#
# Writes: dist/pro-pinball-timeshock-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix pro-pinball-timeshock \
  --exe-name Pro_Pinball__Timeshock__Recompiled \
  --display-name "Pro Pinball: Timeshock! Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Pro Pinball - Timeshock! (USA).cue" \
